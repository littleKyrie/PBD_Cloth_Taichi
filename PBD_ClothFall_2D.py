...
def Start():
    for i, j in X:
        X[i, j] = [i * quad_size - 0.5, j * quad_size + 0.5]
...
def PBD():
    ...
    for e in range(0, e_number[None]/2):
        ...
        #X_new[vi, vj] += 0.5 * (X[vi, vj] + X[ui, uj] + Length[e] * (X[vi, vj] - X[ui, uj]) / currentLength)
        #X_new[ui, uj] += 0.5 * (X[vi, vj] + X[ui, uj] - Length[e] * (X[vi, vj] - X[ui, uj]) / currentLength)
    ...
    for i, j in ti.ndrange(n, n):
        ...
        if i == (n-1) and j == 0:
            ...
            V[i, j] += ((X_new[i, j] + alpha * X[i, j]) / (num[i, j] + alpha) - X[i, j]) / dt
